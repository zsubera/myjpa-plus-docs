...
| `captureAndResetForAsync()` | Capture state for async boundary |
...
| `containsSafeFunction(name)` | Check if function is in safe whitelist |
| `containsBooleanFunction(name)` | Check if function is in boolean whitelist |
...
Fixed-size cache with sampling-based eviction. Used internally by `LambdaUtils` and available as a standalone cache for custom use cases.

| Method | Description |
|--------|-------------|
| `SampledEvictionCache(maxSize, evictionTargetRatio, samplingInterval, initialCapacity)` | Full constructor |
| `SampledEvictionCache(maxSize, evictionTargetRatio, samplingInterval)` | Constructor (default initial capacity 64) |
| `get(key)` | Get value by key |
| `computeIfAbsent(key, mappingFunction)` | Compute-if-absent with sampling eviction |
