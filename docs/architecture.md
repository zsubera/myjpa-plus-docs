...
│   ├── QueryCacheManager — ConcurrentHashMap-based TTL cache (implements CacheAdapter)
...
│
├── projection/        # DTO & Tuple projection (integrated into QuerySpec)
│   └── QueryProjectionSupport — select()/selectAs()/asDto() execution via MyJpaTemplate
│
...
│   ├── SqlSlowQueryInterceptor — Hibernate StatementInspector slow query detection (deprecated)
...
└── util/              # Shared utilities
    ...
    ├── SampledEvictionCache — ConcurrentHashMap-based sample eviction cache
...
CteSpec.as(sql)
  ...
  → SQL injection pattern detection (comments, semicolons, UNION SELECT, WAITFOR DELAY)
...
| QueryProjectionSupport | No | Delegates to QuerySpec (mutable builder) |
...
| QueryCacheManager | Yes | ConcurrentHashMap + ReentrantLock.tryLock() |
| DialectDetector | Yes | ConcurrentHashMap cache |
...
| LambdaUtils | Yes | SampledEvictionCache (ConcurrentHashMap-based) |
| IdentifierValidator | Yes | Volatile flags (write-once at configuration time) |
| SampledEvictionCache | Yes | ConcurrentHashMap + tryLock eviction |
