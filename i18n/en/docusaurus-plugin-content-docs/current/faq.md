...
**Maven spring-boot-maven-plugin configuration:**
```xml
<plugin>
...
### Q: What is the persistence context strategy?

`AbstractBulkOperationSpec.persistenceStrategy()` controls how the persistence context is managed after bulk operations:

```java
// Default — auto clears L1 cache after bulk operations (AUTO_CLEAR)
jpa.execute(jpa.update(User.class).set(User::getStatus, "INACTIVE"));

// Caller manages flush/clear manually (DEFER_TO_CALLER)
...
`DEFER_TO_CALLER` is useful when you need to continue working with entities in the same persistence context after bulk operations.
