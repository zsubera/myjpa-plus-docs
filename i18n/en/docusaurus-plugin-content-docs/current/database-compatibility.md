...
See [MIGRATION.md](https://github.com/zsubera/myjpa-plus/blob/main/MIGRATION.md) for the full migration guide.

Key changes:
- Caffeine cache unification: all hand-written cache implementations replaced with Caffeine
- `persistenceStrategy()` now supports `DEFER_TO_CALLER` mode for `UpdateSpec`, `DeleteSpec`, and `MergeSpec`
- Multiple security and correctness fixes
